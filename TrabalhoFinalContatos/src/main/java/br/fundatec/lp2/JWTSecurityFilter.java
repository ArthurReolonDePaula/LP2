...
public class JWTSecurityFilter implements Filter {
	...
	static {
		...
		loggedInUsers.add("bruna");
		loggedInUsers.add("joao");
	...
	private void validateAdm(String usuario) {
		if (!loggedInUsers.contains(usuario) && !usuario.equals("arthur")) {
			throw new RuntimeException("Usuario mestre invalido");
		}
	}
